...
bool keyW = false, keyA = false, keyS = false, keyD = false;   // wasd 추가 
...
// 장애물 회전 방향 -- 황창일==================================================================================================================================================
struct Obstacle {
    float x, y; //장애물 현재 위치 좌표
    float size; // 장애물 크기
    float angle = 0.0f; // 장애물 현재 회전 각도 
    float rotationSpeed = 1.0f; // 시계방향
    float baseY = 0.0f;       // 원래의 Y 위치
...
void createObstacles() { 
    obstacles.clear();  // 게임을 리셋할때마다 불러옴

    const float size = 60.0f; //모든 장애물 크기

    // --- 아래/위 교차 장애물 6개 -------------------------------------------------
    const int verticalCount = 6;
    const float marginX = 150.0f;  // 화면 양쪽 여백 공간
    const float lowY = 200.0f; //장애물 아래 위치 
    const float highY = HEIGHT - 200.0f; // 장애물 위쪽 위치
    float verticalSpacing = (WIDTH - 2 * marginX) / (verticalCount - 1);  // 장애물 간격 과 수평 설정

    for (int i = 0; i < verticalCount; ++i) {
        Obstacle ob;
        ob.size = size;
        ob.x = marginX + i * verticalSpacing;
        ob.y = (i % 2 == 0) ? lowY : highY;
        ob.angle = static_cast<float>(rand() % 360);   // 회전 각도 랜덤으로 설정 
        ob.rotationSpeed = (rand() % 2 == 0) ? 1.0f : -1.0f; //시계방향 반시계 방향 랜덤 설정

        ob.baseY = ob.y;
        ob.floatPhase = static_cast<float>(rand() % 360);

        obstacles.push_back(ob);
    }

    // --- 중앙 y 라인 장애물 3개 -- 황창일 
    const int centerCount = 3;
    const float centerY = HEIGHT / 2.0f;
    float centerSpacing = (WIDTH - 2 * marginX) / (centerCount + 1);

    for (int i = 1; i <= centerCount; ++i) {
        Obstacle ob;
        ob.size = size;
        ob.x = marginX + i * centerSpacing;
        ob.y = centerY;
        ob.angle = static_cast<float>(rand() % 360);
        ob.rotationSpeed = (rand() % 2 == 0) ? 1.0f : -1.0f;
...
void update() {
    ...
    // 추가: WASD 이동   w누르면 천천히 내려감 
    float moveSpeed = 3.0f;
    if (keyW) characterY += moveSpeed;
    if (keyS) characterY -= moveSpeed;
    if (keyA) characterX -= moveSpeed;
    if (keyD) characterX += moveSpeed;


    if (characterY < 0) characterY = 0;
    if (characterY > HEIGHT) characterY = HEIGHT;

    // 장애물 회전 코드 360도 회전 (void update() 함수 밑에 넣기 )구조물 충돌체크 전 -- 황창일 =================================================================
    for (auto& ob : obstacles) {
        // 회전
        ob.angle += ob.rotationSpeed;
        if (ob.angle >= 360.0f) ob.angle -= 360.0f;
        if (ob.angle < 0.0f) ob.angle += 360.0f;

        // 위아래 움직임 (사인파 기반)
        float time = static_cast<float>(glfwGetTime());
        ob.y = ob.baseY + std::sin(time * 0.6f + ob.floatPhase) * 40.0f;  // 진폭 20픽셀
    }
    //=========================================================================================================================================================
    ...
    //구조물 충돌 체크
    for (auto& ob : obstacles) {
        if (checkCollision(characterX - 25, characterY - 25, 50, 50,
            ob.x - ob.size / 2, ob.y - ob.size / 2, ob.size, ob.size)) {
            if (!isBlinking) {
                playerHP -= 20;
                isBlinking = true;
...
void renderParticles() {
    ...
    //장애물  모양 , 색    (입자렌더링 void renderParticles() 밑에 넣기 ) -- 황창일  ======================================================================================
void renderObstacles() {
    glColor3f(1.0f, 1.0f, 0.0f);  // 노란색

    for (auto& ob : obstacles) {
        float s = ob.size / 2;
        float armWidth = s * 0.3f;
        float armLength = s;

        glPushMatrix();
        glTranslatef(ob.x, ob.y, 0.0f);
        glRotatef(ob.angle, 0.0f, 0.0f, 1.0f); // 회전 적용

        for (int i = 0; i < 4; ++i) {
            glRotatef(90.0f, 0.0f, 0.0f, 1.0f); // 90도씩 회전하면서 팔 4개 그림 바람개비모양

            glBegin(GL_QUADS);
            glVertex2f(-armWidth, 0.0f);
            glVertex2f(armWidth, 0.0f);
            glVertex2f(armWidth, armLength);
            glVertex2f(-armWidth, armLength);
            glEnd();
        }
