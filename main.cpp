...
void createObstacles() {
    ...
    const float margin = 100.0f; // 화면 테두리와의 거리
    const float safeRadius = 200.0f; // 플레이어 주변 안전 반경
    const int maxTries = 100;

    for (int i = 0; i < numObstacles; ) {
        Obstacle ob;
        ob.size = size;
        ob.x = margin + rand() % static_cast<int>(WIDTH - 2 * margin);
        ob.y = margin + rand() % static_cast<int>(HEIGHT - 2 * margin);

        // 플레이어 근처 회피
        float dx = ob.x - characterX;
        float dy = ob.y - characterY;
        if (sqrt(dx * dx + dy * dy) < safeRadius) continue;

        // 기존 장애물과 겹치지 않게
        bool overlaps = false;
        for (const auto& existing : obstacles) {
            float ddx = ob.x - existing.x;
            float ddy = ob.y - existing.y;
            float minDist = ob.size + existing.size;
            if (sqrt(ddx * ddx + ddy * ddy) < minDist) {
                overlaps = true;
                break;
            }
        }
...
void renderObstacles() {
    ...
    for (auto& ob : obstacles) {
        ...
        glPushMatrix();  // 좌표계 저장
        glTranslatef(ob.x, ob.y, 0.0f); // 중심으로 이동
        glRotatef(ob.angle, 0.0f, 0.0f, 1.0f); // 회전

        glBegin(GL_TRIANGLES);
        glVertex2f(0, s);      // 위쪽ㅁ 
        glVertex2f(-s, -s);    // 왼쪽 아래
        glVertex2f(s, -s);     // 오른쪽 아래
        glEnd();
